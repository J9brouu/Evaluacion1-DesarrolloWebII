...
public class PedidoController {
    ...
    @GetMapping("/listar")
    public String listar(Model model, jakarta.servlet.http.HttpSession session, org.springframework.web.servlet.mvc.support.RedirectAttributes ra){
        ...
        com.example.evaluacion1_desarrolloweb.entity.Usuario u = (com.example.evaluacion1_desarrolloweb.entity.Usuario) sessionUser;
        ...
        // exponer usuario de sesión para que fragments/navbar lo muestre correctamente
    ...
    public String listarMisPedidos(HttpSession session, Model model) {
     ...
     @GetMapping("/crear")
    public String crear(@RequestParam(required = false) Long juegoId, Model model, jakarta.servlet.http.HttpSession session) {
        ...
        // si hay sesión, asignar usuario al pedido (para el binding) y exponerlo al modelo
        Object sessionUser = (session != null) ? session.getAttribute("user") : null;
        if (sessionUser != null && sessionUser instanceof com.example.evaluacion1_desarrolloweb.entity.Usuario) {
            pedido.setUsuarioId((com.example.evaluacion1_desarrolloweb.entity.Usuario) sessionUser);
            model.addAttribute("user", sessionUser);
        }
    ...
    @PostMapping("/save")
    public String save(@ModelAttribute Pedido pedido, @RequestParam(required = false) Long juegoId, jakarta.servlet.http.HttpSession session, org.springframework.web.servlet.mvc.support.RedirectAttributes ra) {
        ...
        if (pedido.getUsuarioId() != null && pedido.getUsuarioId().getId() != null) {
            ...
            // si no tiene usuario, intentar asignarlo desde la sesión
            Object sessionUser = (session != null) ? session.getAttribute("user") : null;
            if (sessionUser != null && sessionUser instanceof com.example.evaluacion1_desarrolloweb.entity.Usuario) {
                pedido.setUsuarioId((com.example.evaluacion1_desarrolloweb.entity.Usuario) sessionUser);
            }
    ...
    public String deletePost(@PathVariable Long id, jakarta.servlet.http.HttpSession session, org.springframework.web.servlet.mvc.support.RedirectAttributes ra) {
        Object sessionUser = (session != null) ? session.getAttribute("user") : null;
        if (sessionUser == null) {
            ra.addFlashAttribute("error", "Debes iniciar sesión para eliminar pedidos");
            return "redirect:/login";
        }
        com.example.evaluacion1_desarrolloweb.entity.Usuario u = (com.example.evaluacion1_desarrolloweb.entity.Usuario) sessionUser;
        String rol = u.getRol() != null ? u.getRol().toLowerCase() : "";
